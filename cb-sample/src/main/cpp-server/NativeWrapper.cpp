...
NativeWrapper::NativeWrapper(int argc, ACE_TCHAR * argv[]) : mLog(getLog("NativeWrapper")) {
	mLog.v() << "initializing ORB with " << argc << " arguments at " << std::hex << (void *) argv << " : " << byIndex(argv, argc) << done;
	mOrb = CORBA::ORB_init(argc, argv);
	mLog.v() << "after initialization: " << argc << " arguments at " << std::hex << (void *) argv << " : " << byIndex(argv, argc) << done;
...
NativeWrapper::~NativeWrapper() {
	// TODO: correct cleanup? Is it necessary to deactivate POA?
